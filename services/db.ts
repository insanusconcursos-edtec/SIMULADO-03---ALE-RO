...
export const defaultState: DBState = {
  ...
  appeals: [],
  appealDeadline: '',
  formTitle: 'SIMULADO 03 - ALE RO - (RANKING)', // Atualizado para Simulado 03
  editalTopics: {},
  questionMetadata: {},
};
...
export const getData = async (): Promise<DBState> => {
  try {
    ...
    if (error?.code === 'not-found' || (error?.message && error.message.includes('database (default) does not exist'))) {
        ...
        console.error("AÇÃO NECESSÁRIA: Acesse https://console.firebase.google.com, selecione o projeto 'simulado-03-ale-ro', vá em 'Firestore Database' e clique em 'Criar Banco de Dados'.");
    } else {
